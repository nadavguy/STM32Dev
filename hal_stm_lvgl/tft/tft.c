...
static void ex_disp_flush(lv_disp_drv_t *drv, const lv_area_t *area, uint8_t * color_p);
...
#else
	static lv_color_t buf[TFT_HOR_RES * TFT_VER_RES * 2];
	lv_disp_draw_buf_init(&disp_buf_1, buf, NULL, TFT_HOR_RES * TFT_VER_RES * 2);   /*Initialize the display buffer*/
#endif
...
//	}
	...
	{
//	    uint16_t color_full = (color_p->ch.red << 11) | (color_p->ch.green << 5) | (color_p->ch.blue);
	    nextFrameToDraw[2 * i] = (uint8_t)(color_p) ;
	    nextFrameToDraw[2 * i + 1] = (uint8_t)(color_p + 1);
